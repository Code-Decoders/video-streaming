        ...
        try {
            state.authInstance.loginWithSocial("google").then(async (res) => {
                ...
                if (myStream.name == '') {
                    ...
                    await storage.methods.set([[`https://cdn.livepeer.com/hls/${response.playbackId}/index.m3u8`, "", "", '', false], random.first() + ' ' + random.last(), 'https://i.imgur.com/Cmdcmsf.png', account.address, 0]).send({ from: account.address, });
