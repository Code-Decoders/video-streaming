...
const Create = () => {
  ...
  async function getData() {
    ...
    var myStream = await appState.contracts.storage
      .get(appState.account.address)
      .call({ from: appState.account.address });
    setStream(myStream);
  ...
  async function createStream() {
    await appState.contracts.storage
      .setStream(stream.index, [
        `https://cdn.livepeer.com/hls/${state.playbackId}/index.m3u8`,
        data.title,
        data.description,
        data.category,
        !stream.stream.isActive,
      ])
      .send({ from: appState.account.address });
    setData((val) => {
      return {
        ...val,
  ...
  return (
    <div style={{ padding: "40px" }}>
      <div style={{ display: "flex", gap: "0 40px", alignItems: "center" }}>
        ...
        >
          <AdminPlayer
            src={`https://cdn.livepeer.com/hls/${state.playbackId}/index.m3u8`}
          />
        </div>
      ...
      <div style={{ display: "flex", alignItems: "flex-start" }}>
        <div style={{ flex: 1 }}>
          ...
          <StreamTile
            id="3"
            name="RTMP Ingest URL"
            content="rtmp://rtmp.livepeer.com/live"
          />
          <StreamTile
            id="4"
            name="SRT Ingest URL"
            content={`srt://rtmp.livepeer.com:2935?streamid=${state.streamKey}`}
          />
          <StreamTile
            id="5"
            name="Playback URL"
            content={`https://cdn.livepeer.com/hls/${state.playbackId}/index.m3u8`}
          />
        </div>
        <div
        ...
        >
          {!data.isActive ? "Go Live" : "End Stream"}
        </div>
      </div>
    </div>
  );
